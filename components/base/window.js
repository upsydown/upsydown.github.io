...
export class Window extends Component {
    ...
    componentDidMount() {
        ...
        // compute center position once client-side and after dimensions are set
        // small timeout to ensure setDefaultWindowDimenstion has updated state
        setTimeout(() => {
            try {
                const winW = window.innerWidth;
                const winH = window.innerHeight;
                const wPerc = this.state.width / 100.0;
                const hPerc = this.state.height / 100.0;
                const winPxW = Math.round(winW * wPerc);
                const winPxH = Math.round(winH * hPerc);
                const centerX = Math.max(0, Math.round((winW - winPxW) / 2));
                const centerY = Math.max(0, Math.round((winH - winPxH) / 2));
                // set into state so Draggable can use defaultPosition on initial render
                this.setState({ startX: centerX, startY: centerY });
            } catch (e) {
    ...
    render() {
        // wait until startX/startY are computed (client-side) so Draggable's defaultPosition centers the window
        if (this.state.startX === null || this.state.startY === null) {
            // still initializing; render nothing to avoid incorrect default positioning
            return null;
        }
